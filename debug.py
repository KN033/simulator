...
class Screens:
    ...
    def default_screen (self):
        while True:
            ...
            startButton = Button(pos=(640, 660),text_input="Start",font=self.get_font(44),base_color=(255, 255, 255),hovering_color=(250, 252, 250))

            # Hint Button
            hintButton = Button(pos=(700, 120),text_input="?",font=self.get_font(20),base_color=(255, 255, 255),hovering_color=(250, 252, 250))
            ...
            for event in pygame.event.get():
                ...
                xpos.handle_event(event, self.parameterInput)
                ypos.handle_event(event, self.parameterInput)
                freefall.handle_event(event, self.parameterInput)
                racket_angle.handle_event(event, self.parameterInput)
                length.handle_event(event, self.parameterInput)
                width.handle_event(event, self.parameterInput)
                # box.update()
            ...
            #             sys.exit()
            #     for box in input_boxes:
            #         box.handle_event((pygame.event.get()), self.parameterInput)
            #     box.update()
